...
export default function EmployeeActionsPage() {
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const [actionsResponse, packageActivitiesResponse] = await Promise.all([
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'PLANNED': return <AlertCircle className="w-5 h-5 text-yellow-600" />;
  ...
  return (
    <CustomerLayout title="Mijn Acties" description="Beheer van je eigen acties en taken">
      {isLoading ? (
        ...
        <>
          ...
          <div className="flex justify-between items-center mb-6">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Mijn Acties</h1>
              <p className="text-gray-600">Beheer je eigen acties en taken</p>
            </div>
